...
public abstract class InboundRequestProcessorImpl implements InboundRequestProcessor {
    ...
    protected void start(InboundTask task, String endpointPostfix) {
        ...
        if (coordination) {
            ...
            // is not available. Currently this is only available for file inbound only.
            if (inboundProperties != null && isPinnedServerEnabled(inboundProperties)) {
                if (isPinnedServer(inboundProperties)) {
                    startInboundRunnerThread(task, tenantDomain);
                } else {
    ...
    private void startInboundRunnerThread(InboundTask task, String tenantDomain) {
